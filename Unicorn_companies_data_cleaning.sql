...
--In columns Valuation and Funding we need to separate the numeric values to work with them 
--creating a user defined function to extract numbers from a string
...
--adding a new column for the extracted columns 
alter table Unicorn_Companies$
add Valuation_in_Billions int

--deleting old column
alter table Unicorn_Companies$
drop column Valuation

--inputing the values extracted in the column
update Unicorn_Companies$
set Valuation_in_Billions = dbo.UDF_extractnumbers (Valuation) 
